...
class TopicFormer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.segmentation_folder = config['segmentation_foler'] #config 추가
    ...
    def sample_topic(self, prob_topics, topics, L):
        ...
        return sampled_topics0, sampled_topics1 # [N, L, K], [N, S, K]
    ...
    def forward(self, feat0, feat1, mask0=None, mask1=None):
        ...
        if sampled_topics is not None:
            ...
            s_topics0, s_topics1 = sampled_topics # [N, L, K], [N, S, K]
          
            for i in range(N):
                segmentation_path = os.path.join(self.segmentation_folder, f"segmentation_{i}.txt")
                if os.path.exists(segmentation_path):
                    with open(segmentation_path, 'r') as file:
                        segmentation_mask = torch.tensor([int(x) for x in file.read().split()], device=feat0.device)
                    
                    s_topics0[i] *= segmentation_mask.unsqueeze(0)
                    s_topics1[i] *= segmentation_mask.unsqueeze(0)
